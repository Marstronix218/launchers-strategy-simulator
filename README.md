...
広告からスマートフォンで利用する企業価値の簡易シミュレーターです。利用者は直近1期の売上高・営業利益・最終利益・現預金・減価償却費を入力し、5段階の業績評価から前期・前々期を自動算出します。自動算出値は確認画面で上書きでき、確定した3期分から10,000経路のモンテカルロ・シミュレーションを行います。ログインや会員登録は不要です。
...
公式LINE CTAは `.env.local` の `VITE_LINE_OFFICIAL_URL` で設定します。URL内の `{message}` は、診断完了後の定型メッセージに置き換わります。未設定またはプレースホルダーのままの場合、汎用LINEページへ誤遷移させずCTAを無効表示します。

```dotenv
VITE_LINE_OFFICIAL_URL=https://line.me/R/oaMessage/YOUR_LINE_ID/?{message}
```

診断後のAI簡易分析は、会社分類、5つの任意回答、確定した3期値、計算済みの分布と確率をVercel Function `/api/diagnosis-insight` からOpenAI Responses APIへ渡して解釈します。AIは予測値を計算・変更しません。APIキーはブラウザへ渡さず、`.env.local`（本番ではVercel Environment Variables）のサーバー専用変数として設定してください。
...
OpenAIへのリクエストは `store: false` で送信し、本アプリのデータベースにも診断値を保存しません。会社名はOpenAIへ送信しません。API側の保持条件はOpenAI組織の契約・データ保持設定に従うため、Zero Data Retentionの有効化状況は本番運用前に確認してください。
